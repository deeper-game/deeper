...
impl AABB {
    ...
    pub fn dimensions(&self) -> (u32, u32, u32) {
        let dx = (self.maximum.x - self.minimum.x) as u32;
        let dy = (self.maximum.y - self.minimum.y) as u32;
        let dz = (self.maximum.z - self.minimum.z) as u32;
        (dx, dy, dz)
    }
    ...
    pub fn has_intersection(lhs: &AABB, rhs: &AABB) -> bool {
        lhs.minimum.cmpgt(rhs.maximum).any()
            || lhs.maximum.cmpgt(rhs.minimum).any()
    }

    pub fn intersection(lhs: &AABB, rhs: &AABB) -> Option<AABB> {
        if !AABB::has_intersection(lhs, rhs) { return None; }
        Some(AABB {
            minimum: lhs.minimum.max(rhs.minimum),
            maximum: lhs.maximum.min(rhs.maximum),
        })
    }
...
#[derive(Clone, PartialEq, Eq, Hash)]
struct Map { // TODO: rename to Level once this works
    pub room_boxes: Vec<AABB>,
    pub open_doorways: Vec<Doorway>,
...
fn match_room_against_doorway(
    ...
    let mut result = Vec::new();
    'doorway_loop: for doorway in &room.doorways {
        if !AABB::has_same_shape(&doorway.bounding_box,
                                 &open_doorway.bounding_box) {
            // TODO: allow partial matches in doorway shape
            continue;
        }

        let offset: IVec3 =
            open_doorway.bounding_box.minimum - doorway.bounding_box.minimum;
        let aabb = room.voxels.bounding_box.shift(&offset);

        if let Some(sliced) = map.voxels.slice(&aabb) {
            for pos in sliced.bounding_box.iter() {
                let voxel = sliced.index(&pos);
                let room_voxel = room.voxels.index(&pos);
        ...
        result.push(DoorwayMatch { offset, used_doorway: doorway.clone() });
    }
    result
}

fn room_gluing(starting_room: Room, size: usize, rooms: &[Room]) -> Map {
    let mut map = Map {
        room_boxes: vec![starting_room.voxels.bounding_box.clone()],
        open_doorways: starting_room.doorways,
        voxels: starting_room.voxels,
    };
    ...
    for iter in 0 .. size {
        ...
        for doorway in &room.doorways {
            if doorway != &doorway_match.used_doorway {
                map.open_doorways.push(doorway.clone());
            }
        }
